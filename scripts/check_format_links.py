#!/usr/bin/env python3
"""Health-check the paper-format / template links in data/conferences.yml.
...
(label ``format-check``) listing the broken links — so the "update formats
...
def open_issue(dead: list[dict]) -> None:
    ...
    title = f"Dead format links: {len(dead)} need updating"
    body = (
        "The weekly link check found broken paper-format / template links in "
        "`data/conferences.yml`. Update the `format` field for each:\n\n"
        + "\n".join(f"- **{d['id']}** ({d['title']}): {d['reason']}\n  {d['format']}" for d in dead)
    ...
    q = requests.get(f"{api}/issues", headers=hdrs,
                     params={"state": "open", "labels": "format-check"}, timeout=30)
    if q.ok and any(i["title"].startswith("Dead format links") for i in q.json()):
        print("  format-check issue already open — skipping")
        return
    r = requests.post(f"{api}/issues", headers=hdrs, timeout=30,
                      json={"title": title, "body": body, "labels": ["format-check"]})
...
def main() -> int:
    ...
    for c in docs:
        ...
        if bad:
            dead.append({"id": c["id"], "title": c["title"], "format": url, "reason": reason})

    if dead:
        print(f"\n{len(dead)} dead format link(s):")
        open_issue(dead)
    else:
        print("\nAll format links healthy.")
    return 0
